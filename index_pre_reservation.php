...
$res_no = $_SESSION['reserv_no'];
$q = "SELECT service from reservation_service WHERE reservation_no = $res_no";
$queryRes=mysqli_query($conn,$q);
// $fetch0 = mysqli_fetch_array($queryRes,MYSQLI_ASSOC);
      ...
      echo "<script>alert('Incorrect username or password');window.location.href='index_pre_reservation.php'</script>";
       ...
       {  
        if ($pwd == $fetch['password']){
            ...
            if (count($service_array) == 1 && strcmp($serv['service'], "Overnight Parking") == 0){
                $_SESSION['total_fees'] = 120;
                $_SESSION['hrs_entry'] = 00;
                $_SESSION['mins_entry'] = 00;
                $_SESSION['hrs_exit'] = 12;
                $_SESSION['mins_exit'] = 00;
                $_SESSION['customer_id'] = $fetch['cust_id'];
                $_SESSION['garage_id'] = $garage_id;
                $_SESSION['reservation_no'] = $res_no;
...
<h2><i><b>Please login to proceed</b></i></h2>
<form action="#" name="myForm" method="post" onsubmit="return validateForm()" style="margin-top: 40px;
  ...
  <input type="text" name="usr" class="form-control" placeholder="Username" aria-label="Username" aria-describedby="basic-addon1">
        ...
        <br>
    <h4>Don't have an account? <a href="index_signup.php">Sign Up!</a></h4>
        <script>
